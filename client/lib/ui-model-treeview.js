...
convention:
	...
	* the 'class' names are the required parts;
...
var nodePart = function (el, className, template, before) {
	...
	if (!template.html) {
		if (typeof template.contentHtml !== "undefined") {
			...
			else template = "<span class='" + className + "'>" + className + "</span>";
		}
	}
...
var nodeToExpand = function (el, template) { return nodePart(el, "tree-to-expand", template); }

/*
template:{ (html | contentHtml/content | name, toExpand, toExpandTemplate), childrenTemplate  } | name.
container: the 'elParent' is already a children container
*/
